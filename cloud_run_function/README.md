...
This HTTP Cloud Run function sends a lightweight request to ten Hugging Face
Spaces. It uses each CPU API's `/health` route, Gradio's `/config` route where
available, and the `HF-Drive`, `Dlp-Video-Downloader`, and Media Toolbox root
routes (the Media AI Toolbox GPU root serves its app login page), so the daily
check does not run model inference, download media, or consume ZeroGPU quota.
